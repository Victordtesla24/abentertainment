...
function EventCard({ event, isPast = false }: { event: Event; isPast?: boolean }) {
  return (
    <Link href={`/events/${event.slug}`}>
      <div
        className={`group relative overflow-hidden bg-[#0a3a52]/50 border border-[#CC8A1C]/10 hover:border-[#CC8A1C]/40 transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_8px_32px_rgba(204,138,28,0.15)] ${
      ...
      >
        ...
        <div className="relative h-52 overflow-hidden bg-gradient-to-br from-[#0a3a52] to-[#062434]">
          ...
          {isPast && (
            <div className="absolute top-4 left-4 px-3 py-1 bg-[#7E7180] text-white text-xs font-body font-semibold uppercase tracking-wider">
              Past
            </div>
          )}
        </div>
        ...
        <div className="p-6 space-y-3">
          <h3 className="text-xl font-display font-bold text-white group-hover:text-[#CC8A1C] transition-colors duration-300">
            {event.title}
          </h3>
          <p className="text-[#7E7180] text-sm font-body line-clamp-2">
            {event.description}
          </p>
          <div className="space-y-2 border-t border-[#CC8A1C]/10 pt-3 text-sm text-[#7E7180] font-body">
            <p>{formatDate(event.date)}</p>
            <p>{event.venue}</p>
            {!isPast && (
              <p className="text-[#CC8A1C] font-display font-bold">
...
export default async function EventsPage() {
  ...
  return (
    <main className="bg-[#062434]">
      ...
      <section className="py-24 md:py-32">
        <div className="container-eu">
          ...
          <h1 className="text-4xl md:text-5xl font-display font-bold text-white mb-4">
            Upcoming & Past <span className="text-[#CC8A1C]">Events</span>
          </h1>
          <p className="text-[#7E7180] text-lg font-body max-w-xl">
        ...
        <section className="pb-16">
          <div className="container-eu">
            <div className="bg-[#0a3a52]/40 border border-[#CC8A1C]/10 p-12 text-center">
              ...
              <p className="text-[#7E7180] mt-4 font-body">
                For event inquiries, contact us at{' '}
                <a
                  href={`mailto:${SITE_CONFIG.contact.email}`}
                  className="text-[#CC8A1C] hover:underline"
                >
      ...
      {/* Past */}
      {pastEvents.length > 0 && (
        <section className="pb-24 border-t border-[#CC8A1C]/10 pt-16">
          <div className="container-eu">
            <h2 className="text-2xl font-display font-bold text-white mb-8">
              Past Events
            </h2>
